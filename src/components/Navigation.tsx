...
const navigationItems = [
  ...
  },
  {
    ...
    submenu: [
      ...
      { label: "Jubiya CRFw", href: "/work/jubia" },
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 py-4">
              ...
              >
                {item.submenu ? (
                  <div className="group relative">
                    <Button
                      variant="ghost"
                      className="text-sm font-medium text-white hover:text-accent flex items-center gap-1 hover:bg-white/10 px-4 py-2 rounded-lg"
                    >
                      ...
                      <div className="py-2">
                        {item.submenu.map((subItem) => (
                          <Link
                            key={subItem.label}
                            to={subItem.href}
                            className="block px-4 py-3 text-sm text-white hover:text-accent hover:bg-white/10 rounded-lg mx-2"
                          >
                            {subItem.label}
                          </Link>
                        ))}
                  ...
                  <Link
                    to={item.href}
                    className="text-sm font-medium text-white hover:text-accent hover:bg-white/10 px-3 py-2 rounded-lg relative overflow-hidden group"
                  >
                    {item.label}
        ...
        {/* Mobile Menu */}
        <div
          className={`fixed top-0 right-0 h-full w-80 max-w-[90vw] bg-forest-dark/95 backdrop-blur-xl border-l border-white/30 z-50 transform transition-transform duration-300 ease-in-out lg:hidden ${
                  ...
                  {item.submenu ? (
                    <div>
                      <button
                        ...
                        className="w-full flex items-center justify-between py-4 text-left text-white font-medium hover:text-accent transition-colors duration-200"
                      >
                        ...
                        <div className="pl-4 pb-4 space-y-2">
                          {item.submenu.map((subItem) => (
                            <Link
                              key={subItem.label}
                              to={subItem.href}
                              className="block py-2 text-sm text-white font-medium hover:text-accent transition-colors duration-200"
                              onClick={toggleMobileMenu}
                            >
                              {subItem.label}
                            </Link>
                    ...
                    <Link
                      to={item.href}
                      className="block py-4 text-white font-medium hover:text-accent transition-colors duration-200"
                      onClick={toggleMobileMenu}
                    >
                      {item.label}
                    </Link>
